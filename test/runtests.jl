...
"""
Return the action of A (that is, the trace of (∂H+i[A,H])^2) using matrix operations
"""
...
@testset "ITensorAGP.jl" begin
    ...
    AGP_matrix = adjoint(V) * AGP_matrix * V
    AGP_matrix[diagind(AGP_matrix)] .= 0.0

    @test S_matrix(AGP_matrix, H_matrix, dH_matrix) ≈
          S_matrix(exactAGP, H_matrix, dH_matrix) rtol = 1e-3
end
